    ...
    .filter('sumPoints', function () {
        return function (data, key) {
            ...
            for (var i = 0; i < data.length; i++) {
                sum = sum + (data[i][key] == 'Р‘РµР· РѕС†РµРЅРєРё' ? 0 : parseInt(data[i][key]));
            }
            return sum;
        }
    }).
                        ...
                        response.issue.forEach(function (value) {
                            ...
                            if ((maxPoint != -1 || $scope.allow_null_points) && assignee != '') {
                                ...
                                $scope.issuesMap[assignee].push({
                                    id: value.id,
                                    maxPoint: maxPoint == -1 ? 'Р‘РµР· РѕС†РµРЅРєРё' : maxPoint,
                                    totalPoint: totalPoint == -1 ? 'Р‘РµР· РѕС†РµРЅРєРё' : totalPoint,
                                    summary: summary
                                });
                            }
                        });
